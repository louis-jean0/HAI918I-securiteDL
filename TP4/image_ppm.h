...
/*===========================================================================*/
void reLU(OCTET* image, int nTaille) {
   for(int i = 0; i < nTaille; ++i) {
      if(image[i] < 0) {
         image[i] = 0;
      }
...
/*===========================================================================*/
void appliquer_filtre_3x3(OCTET* image, OCTET* sortie, short* filtre, int nH, int nW) {
   for(int i = 1; i < nH - 1; ++i) {
      for(int j = 1; j < nW - 1; ++j) {
         OCTET current = image[indiceImage(i,j,nW)];
         OCTET voisin_HG = image[indiceImage(i-1,j-1,nW)];
         OCTET voisin_UP = image[indiceImage(i-1,j,nW)];
         OCTET voisin_HD = image[indiceImage(i-1,j+1,nW)];
         OCTET voisin_G = image[indiceImage(i,j-1,nW)];
         OCTET voisin_D = image[indiceImage(i,j+1,nW)];
         OCTET voisin_BG = image[indiceImage(i+1,j-1,nW)];
         OCTET voisin_DOWN = image[indiceImage(i+1,j,nW)];
         OCTET voisin_BD = image[indiceImage(i+1,j+1,nW)];
         OCTET valeur = filtre[0] * voisin_HG + filtre[1] * voisin_UP + filtre[2] * voisin_HD
                                             + filtre[3] * voisin_G + filtre[4] * current + filtre[5] * voisin_D
                                             + filtre[6] * voisin_BG + filtre[7] * voisin_DOWN + filtre[8] * voisin_BD;
         sortie[indiceImage(i-1,j-1,nW-2)] = valeur + 128;//(OCTET)std::min(std::max(valeur, 0), 255);
      }  
   }
}
/*===========================================================================*/
...
/*===========================================================================*/
void max_pooling_2x2(OCTET* image, OCTET* image_pooled, int nH, int nW) {
    int pooled_nH = nH / 2;
    int pooled_nW = nW / 2;
    for(int i = 0; i < pooled_nH; ++i) {
        for(int j = 0; j < pooled_nW; ++j) {
            OCTET max_val = 0;
            for(int di = 0; di < 2; ++di) {
                for(int dj = 0; dj < 2; ++dj) {
                    int idx = indiceImage(2*i + di, 2*j + dj, nW);
                    if(image[idx] > max_val) {
                        max_val = image[idx];
                    }
                }
            }
            image_pooled[indiceImage(i, j, pooled_nW)] = max_val;
        }
    }
}
/*===========================================================================*/